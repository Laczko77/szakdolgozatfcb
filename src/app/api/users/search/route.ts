 ...
 *       - max 20 találat
 *       - csak publikus mezők: id, username, avatar_url
 *       - a hívó user nem szerepel a találatokban
 *       - üres / túl rövid q → üres lista
 *
 * Auth: bejelentkezett userek számára érhető el (a DM workflow része).
 */
...
export async function GET(request: NextRequest) {
  ...
  // Postgres ILIKE wildcard escape — `%` és `_` és `\` a literal-okhoz.
